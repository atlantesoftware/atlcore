...
from atlcore.schedule.periods import weekday_names, weekday_abbrs,  Month

register = template.Library()

class build_range(template.Node):
    def __init__(self, value, list):
        self.value = value
        self.list = list
        
    def render(self, context):
        context[self.list] = range(0, self.value) 
        return ''


def get_range(parser, token):
    """
    Showing a number as a list like range do
    ...
    return build_range(int(tokens[1]), tokens[3])

register.tag(get_range)

@register.inclusion_tag("site/atl_schedule/schedule/_month_table.html",  takes_context=True)
...
@register.inclusion_tag("site/atl_schedule/schedule/_day_cell.html",  takes_context=True)
...
@register.inclusion_tag("site/atl_schedule/schedule/_daily_table.html", takes_context=True)
...
@register.inclusion_tag("site/atl_schedule/schedule/_event_title.html", takes_context=True)
...
@register.inclusion_tag("site/atl_schedule/schedule/_event_options.html", takes_context=True)
...
@register.inclusion_tag("site/atl_schedule/schedule/_create_event_options.html", takes_context=True)
...
@register.inclusion_tag("site/atl_schedule/schedule/_prevnext.html")
...
@register.inclusion_tag("site/atl_schedule/schedule/_detail.html")
