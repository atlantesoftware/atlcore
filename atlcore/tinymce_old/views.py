...
from contenttype.models.node import Node
from django.db.models import Q
from contenttype.models.folder import Folder
from contenttype.models.image import Picture
...
def imagebrowser(request):
    ...
    node_id = request.GET.get('id','0')
    if not (node_id == '0') and not (node_id == ''):
        folder_list = Folder.objects.filter(parent=node_id).filter(Q(language=language) | Q(neutral=True)).order_by('-order')
        picture_list = Picture.objects.filter(parent=node_id).filter(Q(language=language) | Q(neutral=True)).order_by('-order')
        node = Folder.objects.get(pk=node_id)
        parent_folder = node.get_parent() 
    else:
        folder_list = Folder.objects.filter(parent=None).filter(Q(language=language) | Q(neutral=True)).order_by('-order')
        picture_list = Picture.objects.filter(parent=None).filter(Q(language=language) | Q(neutral=True)).order_by('-order')
        parent_folder = None
    ...
    return render_to_response('tinymce/image_browser.html', context)


def contentbrowser(request):
    language = get_language()
    node_id = request.GET.get('id','0')
    if not (node_id == '0') and not (node_id == ''):
        folder_list = Folder.objects.filter(parent=node_id).filter(Q(language=language) | Q(neutral=True)).order_by('-order')
        picture_list = Content.objects.filter(parent=node_id).filter(Q(language=language) | Q(neutral=True)).order_by('-order')        
        content_list = Content.objects.filter(parent=node_id).filter(Q(language=language) | Q(neutral=True)).order_by('-order')        
        parent_folder = Container.objects.get(pk=node_id)
    else:
        folder_list = Folder.objects.filter(parent=None).filter(Q(language=language) | Q(neutral=True)).order_by('-order')
        picture_list = Content.objects.filter(parent=None).filter(Q(language=language) | Q(neutral=True)).order_by('-order')
        content_list = Content.objects.filter(parent=None).filter(Q(language=language) | Q(neutral=True)).order_by('-order')
        parent_folder = None    
    ...
    return render_to_response('tinymce/content_browser.html', context)
